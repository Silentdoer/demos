 ...
 * @since 3.0
 */
// TODO annotation-driven 确实是3.0就有了
class AnnotationDrivenBeanDefinitionParser implements BeanDefinitionParser {
	...
	// TODO <mvc:annotation-driven
	// TODO driven后确实会添加默认的 HttpMessageConverter以及HandlerMethodArgumentResolver和HandlerMethodReturnValueHandler
	// TODO ，但似乎不会添加到beanFactory？？（或者实例化这些组件的时候会加进去？？）
	@Override
	public BeanDefinition parse(Element element, ParserContext parserContext) {
		...
		// 将annotation-driven这个元素作为containingBean之一
		parserContext.pushContainingComponent(compDefinition);

		// negotiation 谈判、转让、协商（这个似乎是多视图的协商管理器）TODO， 注意这个是一定会存在的，就像RequestMappingHandlerAdapter一样
		// <mvc:annotation-driven content-negotiation-manager="xxxmanager" />
		RuntimeBeanReference contentNegotiationManager = getContentNegotiationManager(element, source, parserContext);

		RootBeanDefinition handlerMappingDef = new RootBeanDefinition(RequestMappingHandlerMapping.class);
		handlerMappingDef.setSource(source);
		// RequestMappingHandlerMapping 作为基础设施优先初始化？（貌似不会只是一个描述信息，也就是修改对应BeanDefinition会有用）
		handlerMappingDef.setRole(BeanDefinition.ROLE_INFRASTRUCTURE);
		// TODO RequestMappingHandlerMapping 的order 默认是0
		...
		// 配置<mvc:path-matching
		configurePathMatchingProperties(handlerMappingDef, element, parserContext);

		// TODO 开启了annotation-driven后 不要主动添加 RequestMappingHandlerMapping 和RequestMappingHandlerAdapter
		// beanName就是RequestMappingHandlerMapping的类名
		...
		// TODO 这个是全局类型转换的，类似DataBinder，比如默认将所有的Date转换为什么格式
		...
		// TODO 重要，HttpMessageConverter，返回的是持有 所有BeanDefinitionHolder的列表（内部会处理register-default）
		// 默认有StringHttpMessageConverter、ByteArrayHttpMessageConverter等，ManagedList继承了ArrayList
		ManagedList<?> messageConverters = getMessageConverters(element, source, parserContext);

		// TODO PropertyValue的Value 1可以是TypeStringValue、BeanReference、BeanDefinition ？？、BeanDefinitionHolder？？

		// TODO HandlerMethodArgumentResolver
		ManagedList<?> argumentResolvers = getArgumentResolvers(element, parserContext);

		// TODO HandlerMethodReturnValueHandler
		ManagedList<?> returnValueHandlers = getReturnValueHandlers(element, parserContext);

		// TODO 下面这四个都是要支持 async-support的前提
		// async-support 之后 业务处理response超时时间？？RequestMappingHandlerAdapter的asyncRequestTimeout属性
		String asyncTimeout = getAsyncTimeout(element);
		RuntimeBeanReference asyncExecutor = getAsyncExecutor(element);  // TODO 这两个暂且不管
		ManagedList<?> callableInterceptors = getCallableInterceptors(element, source, parserContext);
		ManagedList<?> deferredResultInterceptors = getDeferredResultInterceptors(element, source, parserContext);

		// TODO adapter里也有 contentNegotiationManager
		RootBeanDefinition handlerAdapterDef = new RootBeanDefinition(RequestMappingHandlerAdapter.class);
		handlerAdapterDef.setSource(source);  // source就是指这个BeanDefinition配置的来源（这里是用个element对象表示）
		handlerAdapterDef.setRole(BeanDefinition.ROLE_INFRASTRUCTURE);
		// TODO 注意，RequestMappingHandlerAdapter里也有contentNegotiationManager
		handlerAdapterDef.getPropertyValues().add("contentNegotiationManager", contentNegotiationManager);
		handlerAdapterDef.getPropertyValues().add("webBindingInitializer", bindingDef);

		// TODO 只有RequestMappingHandlerAdapter里有messageConverters，然后MethodArgument和ReturnValue里要用到
		//List<HttpMessageConverter<?>> messageConverters;
		handlerAdapterDef.getPropertyValues().add("messageConverters", messageConverters);

		// 在读取RequestBody之前的advice，默认是JsonViewRequestBodyAdvice
		addRequestBodyAdvice(handlerAdapterDef);
		// 在写ResponseBody之前的advice，默认是JsonViewResponseBodyAdvice
		...
		// TODO 自定义的HandlerMethodArgumentResolver
		if (argumentResolvers != null) {
			handlerAdapterDef.getPropertyValues().add("customArgumentResolvers", argumentResolvers);
		}
		// TODO 自定义的HandlerMethodReturnValueHandler
		...
		// TODO 向beanFactory注册 RequestMappingHandlerAdapter 对象
		...
		// TODO 异常处理的 HandlerExceptionResolver
	...
	private RuntimeBeanReference getContentNegotiationManager(Element element, Object source,
		...
		if (element.hasAttribute("content-negotiation-manager")) {
			...
			//名称： mvcContentNegotiationManager
			String name = CONTENT_NEGOTIATION_MANAGER_BEAN_NAME;

			// TODO 原来如此 ContentNegotiationManagerFactoryBean 在annotation-driven后是一定会有一个的，这里注册到beanFactory里
			parserContext.getReaderContext().getRegistry().registerBeanDefinition(name , factoryBeanDef);

			// TODO 这个是BeanDefinitionHolder ，这个似乎是将此bean加入到当前containingBean（annotation-driven）的nested component里？
	...
	// TODO 目前而言这个似乎是专门用来处理 RequestMappingHandlerMapping 的
	private void configurePathMatchingProperties(RootBeanDefinition handlerMappingDef, Element element,
			ParserContext parserContext) {

		// TODO 如<mvc:annotaion-driven 内部的子元素<mvc:path-matching
	...
	private ManagedList<?> getMessageConverters(Element element, Object source, ParserContext parserContext) {
		...
		if (convertersElement != null) {
			...
			for (Element beanElement : DomUtils.getChildElementsByTagName(convertersElement, "bean", "ref")) {
				...
				// TODO object 是一个BeanDefinitionHolder对象
				Object object = parserContext.getDelegate().parsePropertySubElement(beanElement, null);
				messageConverters.add(object);
			}
		}

		// TODO 重要，当他是false时则不注册默认的HttpMessageConverter
