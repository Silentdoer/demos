...
public class ServletInvocableHandlerMethod extends InvocableHandlerMethod {
	...
	private void initResponseStatus() {
		// TODO 假设@RequestMapping方法上还有@ResponseStatus
		ResponseStatus annotation = getMethodAnnotation(ResponseStatus.class);
		if (annotation == null) {
			// TODO 获取直接注解在controller上的@ResponseStatus注解
			annotation = AnnotatedElementUtils.findMergedAnnotation(getBeanType(), ResponseStatus.class);
		}
		if (annotation != null) {
			// TODO 这个应该是默认值（可能为null）
	...
	public void invokeAndHandle(ServletWebRequest webRequest,
		...
		else if (StringUtils.hasText(this.responseReason)) {  // 这个是产生了异常，且此异常被ResponseStatus注解过了（会填reason成员值）？
			mavContainer.setRequestHandled(true);
			return;
		}

		// DispatcherServlet就是通过这个来决定是否需要进一步处理返回数据？？
		mavContainer.setRequestHandled(false);
		try {
			// 假设没有用@RequestBody（如果用了则handled为true，调用此方法的会返回null）
