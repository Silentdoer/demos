...
public abstract class AbstractMessageConverterMethodProcessor extends AbstractMessageConverterMethodArgumentResolver
	...
	protected <T> void writeWithMessageConverters(T value, MethodParameter returnType,
		...
		// 获取根据策略 被允许的 MediaType（而不一定是request的Accept要根据ContentNegotiationStrategy的策略来返回，故
		// 这一块能够返回正确的数据是不可靠的，而且万一客户端的Accept没有设置呢？而我这边则一定要返回固定的字符集数据）
		// TODO 不过可以通过filter 在request的Accept加入默认值（额，只能读不能设置。。）
		List<MediaType> requestedMediaTypes = getAcceptableMediaTypes(request);
		// TODO 返回的是produces里的MediaType或者是配置的所有canWrite(valueClass, null) 的 Converter的supportedMediaTypes
		// 如果返回的是字符串，显然canWrite是true
		...
		for (MediaType requestedType : requestedMediaTypes) {
			for (MediaType producibleType : producibleMediaTypes) {
				if (requestedType.isCompatibleWith(producibleType)) {
					// getMostSpecificMediaType是说获取二者当中描述最详细的那个，比如同样是text/plain一个有字符集一个没有
		...
		// TODO 按优先描述详细程度和然后权重的规则排序
		MediaType.sortBySpecificityAndQuality(mediaTypes);

		MediaType selectedMediaType = null;
		for (MediaType mediaType : mediaTypes) {
			if (mediaType.isConcrete()) {  // 判断这个MediaType不是 “通配符”，由于已经排序，故最先符合的就是最匹配的最详细最高优先级的
		...
		if (selectedMediaType != null) {
			...
			for (HttpMessageConverter<?> messageConverter : this.messageConverters) {
				// TODO GenericHttpMessageConverter是用来处理泛型数据的（或者说返回的值的类型原本是泛型？？）
				if (messageConverter instanceof GenericHttpMessageConverter) {
					if (((GenericHttpMessageConverter) messageConverter).canWrite(
							declaredType, valueType, selectedMediaType)) {
						// 这个Advice()其实就是我们配置的实现了特定接口的bean（是一个command对象）
				...
				// wildcard 通配符
				// TODO 大多数情况都是看这里，注意这里canWrite会对selectedMediaType进行再一次判断，如果它不存在于自己的supportedMediaTypes里返回false
					...
					if (outputValue != null) {
						addContentDispositionHeader(inputMessage, outputMessage);
						// TODO selectedMediaType至关重要（不为null且里面有字符集如text/plain;charset=utf-8），它不能为null，否则就默认用ISO-8859-1了
	...
	protected List<MediaType> getProducibleMediaTypes(HttpServletRequest request, Class<?> valueClass, Type declaredType) {
		// TODO 这个获取的就是 RequestMapping里的produces配置的，从这段代码看配置了produces那么converter的supportedMediaTypes就没用了
