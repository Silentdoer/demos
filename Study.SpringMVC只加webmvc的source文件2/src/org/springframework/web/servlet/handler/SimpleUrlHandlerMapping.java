 ...
 */
// TODO 注意，这个返回的handler未必就一定是 HttpRequestHandler 对象，其实还可以是自定义的bean，只要配置能处理
	// TODO 此bean 的HandlerAdapter即可（这个HandlerMapping没有实现InitializingBean，故不能在方法初始化时去探测
	// TODO 实现了HttpRequestHandler的bean，但是也不能，因为此HandlerMapping不只是可以返回这种类型的handler）
	// 看了下最好不要用<mvc:default-servlet-handler>和<mvc:resources mapping="" location="">因为它们可能产生多个SimpleUrlHandlerMapping对象
public class SimpleUrlHandlerMapping extends AbstractUrlHandlerMapping {
	 ...
	 * @see #setUrlMap
	 */
	// TODO 配置里重要的 proprerty ，但这里只是将配置的properties对象放到urlMap里
	// TODO Merge往往是 存在一样的，则保留原来的，而set则是直接覆盖
	 ...
	 * @see #setMappings
	 */
	// 还可以直接 <property name="urlMap"><map><entry key= value= /></map></property>
	 ...
	 */
	// TODO 这个是在 bean创建后赋值后初始化之前 调用的（故它的属性已经初始化所以setMappings已经执行，故urlMap存储了所有的props）
	@Override
	...
	protected void registerHandlers(Map<String, Object> urlMap) throws BeansException {
		if (urlMap.isEmpty()) {
			...
			for (Map.Entry<String, Object> entry : urlMap.entrySet()) {
				...
				// handler 是 HttpRequestHandler
				Object handler = entry.getValue();
				// Prepend with slash if not already present.
				if (!url.startsWith("/")) {
					// TODO 所以 其实 mapping="/static/**"和"static/**"是一样的，只不过前者是规范写法
					url = "/" + url;
				}
