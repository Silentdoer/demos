...
public abstract class FrameworkServlet extends HttpServletBean implements ApplicationContextAware {
	 ...
	 */
	// TODO 重要方法，用来设置servlet中的beans的配置文件
	...
	protected final void initServletBean() throws ServletException {
		...
		try {
			// TODO 对于Web应用是用WebApplicationContext，对于Java Application一般用ClassPathXmlApplicationContext或FileSystemXmlApplicationContext
	...
	protected WebApplicationContext initWebApplicationContext() {
		//TODO 第一次调用这里应该是null，这个是作为parent
		WebApplicationContext rootContext =
				WebApplicationContextUtils.getWebApplicationContext(getServletContext());
		WebApplicationContext wac = null;

		// 除非这个Servlet是我们手动创建的，否则webApplicationContext在这个时候一定是 null
		...
		if (wac == null) {
			// TODO 一般都会进入，除非是自己手动创建的servlet
			// No context instance was injected at construction time -> see if one
			// has been registered in the servlet context. If one exists, it is assumed
			// that the parent context (if any) has already been set and that the
			// user has performed any initialization such as setting the context id
			// TODO 如果之前这个context里有某个servlet已经初始化了，那么这里是可能找得到的（可以手动主动缓存在context）。
			wac = findWebApplicationContext();
		}
		if (wac == null) {
			// TODO，假设只有一个 servlet ，那么会执行这里（或者是第一个初始化的Servlet）
			// No context instance is defined for this servlet -> create a local one
			// TODO  重要，目前而言绝大多数情况都是在这里初始化的Spring的
			wac = createWebApplicationContext(rootContext);
		}

		//TODO onApplicationEvent(ContextRefreshedEvent event) 执行后 received会是true
		// TODO 注意DispatcherServlet并没有继承ApplicationListener接口，故观察者并不是servlet
		// TODO ，观察者是 FrameworkServlet 的内部类ContextRefreshListener，然后它的onApplicationEvent执行后转由Servlet的这个方法执行
		...
		if (this.publishContext) {  // 默认是true，这个是用来缓存Springcontext到ServletContext里
	...
	protected WebApplicationContext findWebApplicationContext() {
		// TODO 应该是 null，因为Spring context是属于整个webapp的，故要缓存也是缓存在servletcontext里
	...
	protected WebApplicationContext createWebApplicationContext(ApplicationContext parent) {
		Class<?> contextClass = getContextClass();  // 貌似默认就是 XmlWebApplicationContext
		if (this.logger.isDebugEnabled()) {
		...
		// TODO 这里，用 XmlWebApplicationContext 默认构造方法创建一个类
		ConfigurableWebApplicationContext wac =
				(ConfigurableWebApplicationContext) BeanUtils.instantiateClass(contextClass);
//XmlWebApplicationContext
		wac.setEnvironment(getEnvironment());  // 用于配置一些环境变量，如ServletContext、ServletConfig等
		wac.setParent(parent);  // null
		// 对于ClassPathXmlApplicationContext而言configLocation是来自构造方法然后直接调用refresh()方法。

		// TODO 内部会对如 classpath:..., classpath:... 进行分解成为多个location
		wac.setConfigLocation(getContextConfigLocation());
		//AbstractApplicationContext c;

		// TODO 内部会对wac添加ApplicationListener，以及wac.refresh()，refresh()内部会loadBeanDefinition
		configureAndRefreshWebApplicationContext(wac);
//FileSystemXmlApplicationContext c;
		/*ClassPathXmlApplicationContext c;
		c.setConfigLocation("classpath:...; classpath:...");
		c.refresh();*/
		return wac;
	}
	...
	protected void configureAndRefreshWebApplicationContext(ConfigurableWebApplicationContext wac) {
		...
		// TODO 要refresh()完成后才最终会调用ContextRefreshListener的onApplicationEvent
		// TODO 装饰者模式。
		...
		if (env instanceof ConfigurableWebEnvironment) {  // true，不管
			((ConfigurableWebEnvironment) env).initPropertySources(getServletContext(), getServletConfig());
		}

		postProcessWebApplicationContext(wac);  // 空实现

		// TODO 为wac添加ApplicationContextInitializer监听器
		applyInitializers(wac);

		// TODO 重要方法，这个方法是ApplicationContext的启动方法，类似start/init
		wac.refresh();
	}
	 ...
	 * @return the WebApplicationContext for this servlet
	 * @see XmlWebApplicationContext 来自web的子包
	 * @see #createWebApplicationContext(ApplicationContext)
	 */
	protected WebApplicationContext createWebApplicationContext(WebApplicationContext parent) {
		// TODO 重要，parent基本上都是null
	...
	protected void applyInitializers(ConfigurableApplicationContext wac) {
		// TODO 应该返回的是null ，globalInitializerClasses
		String globalClassNames = getServletContext().getInitParameter(ContextLoader.GLOBAL_INITIALIZER_CLASSES_PARAM);
		if (globalClassNames != null) {  // 不进
			for (String className : StringUtils.tokenizeToStringArray(globalClassNames, INIT_PARAM_DELIMITERS)) {
				this.contextInitializers.add(loadInitializer(className, wac));
			}
		}

		if (this.contextInitializerClasses != null) {  // 不进
	...
	public void onApplicationEvent(ContextRefreshedEvent event) {
		// TODO 查下为什么SpringMVC4.3不需要继承ApplicationListener接口？？
		// TODO 由这个标识可猜 这个事件应该是只被调用一次
		// ContextRefreshedEvent事件是指 当applicationContext 将BeanDefinition和Bean都加载完毕会产生的事件（故应该是一次）
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		System.out.println("FrameworkServlet#service(..) in");
