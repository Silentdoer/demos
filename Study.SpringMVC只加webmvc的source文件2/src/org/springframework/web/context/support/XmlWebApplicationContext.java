...
public class XmlWebApplicationContext extends AbstractRefreshableWebApplicationContext {
	...
	protected void loadBeanDefinitions(DefaultListableBeanFactory beanFactory) throws BeansException, IOException {
		...
		// TODO 这个beanFactory是target，即ApplicationContext.getBean委托这个来弄，它是DefaultListableBeanFactory（直接new的）
		XmlBeanDefinitionReader beanDefinitionReader = new XmlBeanDefinitionReader(beanFactory);

		// Configure the bean definition reader with this context's
		// resource loading environment.
		beanDefinitionReader.setEnvironment(getEnvironment());
		beanDefinitionReader.setResourceLoader(this);  // 用来将 bean配置文件路径转换为 流数据的接口
		beanDefinitionReader.setEntityResolver(new ResourceEntityResolver(this));

		// Allow a subclass to provide custom initialization of the reader,
		// then proceed with actually loading the bean definitions.
		initBeanDefinitionReader(beanDefinitionReader);  // 空实现，自己重新的话可以用来存储beanDefinitionReader
...
// 允许格式为classpath*:..xml, classpath*:...xml, classpath:..xml格式的配置。
				// TODO new出来的，里面的beanFactory也是new的，还有个ResourceLoader是set的
				reader.loadBeanDefinitions(configLocation);  // 会返回解析到了多少 BeanDefinition
			}
		}
	}
