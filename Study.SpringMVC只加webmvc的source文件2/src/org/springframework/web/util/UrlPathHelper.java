...
public class UrlPathHelper {
	...
	public String getLookupPathForRequest(HttpServletRequest request) {
		...
		if (this.alwaysUseFullPath) {  // TODO 即用完整的URL而非 context后面的，默认false，但是如果只有context则用URI？
			return getPathWithinApplication(request);
		}
		// Else, use path within current servlet mapping if applicable
		// TODO 如果只有context路径则用/context ，否则用 /servlet/pathInfo ？
	...
	public String getPathWithinServletMapping(HttpServletRequest request) {
		// TODO 貌似忽略大小写？？（/context或/servlet/pathInfo，看访问的是否是根路径）
		String pathWithinApp = getPathWithinApplication(request);
		String servletPath = getServletPath(request);  // 如/servlet（后面应该没有/）
		// sanitized 使清洁（规范化，如有//则合并为/
		String sanitizedPathWithinApp = getSanitizedPath(pathWithinApp);
		String path;

		// If the app container sanitized the servletPath, check against the sanitized version
		if (servletPath.contains(sanitizedPathWithinApp)) {
			// TODO 会匹配到这里一般是 pathInfo为null的情况，如url-pattern是*.do之类的
			path = getRemainingPath(sanitizedPathWithinApp, servletPath, false);
		}
		else {
			// TODO 返回/context 或 /pathInfo
