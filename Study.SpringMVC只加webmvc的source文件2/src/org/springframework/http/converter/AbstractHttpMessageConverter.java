...
public abstract class AbstractHttpMessageConverter<T> implements HttpMessageConverter<T> {
	...
	public Charset getDefaultCharset() {
		System.out.println("springmvc-default-charset:" + defaultCharset.toString());
	...
	public final void write(final T t, MediaType contentType, HttpOutputMessage outputMessage)
		...
		// contentType不可为null，且里面要包括charset，否则最终都会被设置为ISO-8859-1
		addDefaultHeaders(headers, t, contentType);

		// TODO 怕返回的字符串被以ISO-8859-1写入其实可以直接返回字节数组即可
		// 或者改一改StringHttpMessageConverter的源码
		if (outputMessage instanceof StreamingHttpOutputMessage) {  //TODO octet-stream的情况下会来这里（即t本身就是流数据、如字节数组
		...
		else {  // TODO method返回的数据如果不是字节数组或流数据的话都来这里
	...
	protected void addDefaultHeaders(HttpHeaders headers, T t, MediaType contentType) throws IOException{
		if (headers.getContentType() == null) {
			// TODO 这一步至关重要，只要contentType不为null，那么StringHttpMessageConverter写数据时的编码就是contentType
