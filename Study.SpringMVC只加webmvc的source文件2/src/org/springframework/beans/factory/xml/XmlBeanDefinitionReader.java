...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	 ...
	 */
	// TODO 重要，NamespaceHandlerResolver的起点之一，注意这里返回的值是根据resource的配置找到了多少新的 BeanDefinition
	@Override
	...
	public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
		...
		try {
			...
			try {
				...
				if (encodedResource.getEncoding() != null) {  // 应该是null
	 ...
	 */
	// TODO DefaultNamespaceHandlerResolver加载的起点之一，inputSource应该就是beans.xml
	// TODO ，而resource貌似就是类似xsd或dtd文件（好像又确实是beans.xml，只不过前者已经转换为了流数据，后者还是beans.xml的文件路径)
	protected int doLoadBeanDefinitions(InputSource inputSource, Resource resource)
			throws BeanDefinitionStoreException {
		try {

			// TODO 只是单纯的从 XML文件里 获取 Document 对象（但是会通过dtd、xsd来验证配置是否符合要求）
			Document doc = doLoadDocument(inputSource, resource);

			// TODO 重要，将doc内的所有定义进行转换，resource似乎可以不要？？
	...
	protected Document doLoadDocument(InputSource inputSource, Resource resource) throws Exception {
		// TODO 这里面Document、EntityResolver、InputSource都是xml里的公共api不是spring的，EntityResolver就是获取dtd、xsd文件的一个类
	 ...
	 */
	// TODO 这个Document是符合org.w3c.dom 的
	public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {
		// TODO DefaultBeanDefinitionDocumentReader 对象，用于从document对象里读取出（解析出）BeanDefinition对象
		BeanDefinitionDocumentReader documentReader = createBeanDefinitionDocumentReader();


		// TODO 似乎BeanDefinition 就是存储在 XMLBeanDefinitionReader里 ？？（应该是，BeanDefinition和Bean存放的位置不一样
		// TODO ，而XMLBeanDefinitionReader对象和另一个存储Bean的对象都是beanFactory的组件（好吧似乎beanFactory实现了registry的接口）
		// TODO 获取当前BeanDefinition的registry里有多少个BeanDefinition对象
		// TODO 注意，这个registry对象其实就是 DefaultListableBeanFactory ，它实现了BeanDefinitionRegistry接口（registry可以理解为map）
		int countBefore = getRegistry().getBeanDefinitionCount();

		// TODO ReaderContext很重要，这里是真正开始通过Document对象来产生BeanDefinition的地方（怎么解析Element貌似是通过Parser）
		// TODO createReaderContext(..)里 会调用   ：：：：  getNamespaceHandlerResolver() ，这里会创建DefaultNamespaceHandlerResolver对象
		// TODO 重要，这个是真正来注册  BeanDefinition的地方。
		// TODO 注意，这里创建的 XmlReaderContext对象保留有XmlBeanDefinitionReader的引用，而XmlBeanDefinitionReader又有beanFactory的引用
		documentReader.registerBeanDefinitions(doc, createReaderContext(resource));
		// TODO 上面创建的XmlReaderContext类对象很重要
	 ...
	 */
	// TODO 重要，是能够解析beans.xml中有名称空间的Element的起点（Element是否能获取到不归spring管，至少是没什么特殊性）
	public XmlReaderContext createReaderContext(Resource resource) {  // TODO 这个resource应该就是 classpath:mapper/...xml??（好像不是）
	...
	public NamespaceHandlerResolver getNamespaceHandlerResolver() {
		if (this.namespaceHandlerResolver == null) {
			...
			// TODO 重要，可以处理 <mvc:...的namespace，但是具体由哪个处理又需要resolver搜索对应的handler处理
