...
public abstract class AbstractBeanDefinitionReader implements EnvironmentCapable, BeanDefinitionReader {
	...
	// TODO NamespaceHandlerResolver的起点之一
	@Override
	public int loadBeanDefinitions(Resource... resources) throws BeanDefinitionStoreException {
		Assert.notNull(resources, "Resource array must not be null");
		int counter = 0;
		for (Resource resource : resources) {
			// 源头之一，XmlBeanDefinitionReader
			counter += loadBeanDefinitions(resource);  // 这个resource就是真正的对应单个文件了
		}
		return counter;
	}
	...
	public int loadBeanDefinitions(String location, Set<Resource> actualResources) throws BeanDefinitionStoreException {
		...
		if (resourceLoader instanceof ResourcePatternResolver) {
			...
			try {
				// TODO 貌似是搜索 配置文件的（因为配置文件里可以有通配符），是ResourceLoader的子孙接口，多了个getResources
				// TODO location 是 classpath*:..的格式，这里是可能匹配到多个文件类似*Mapper.xml一样符合条件的不止一个
				//ClassPathXmlApplicationContext 构造方法的locations 中的每个元素也是可以是classpath*:..*Context.xml格式的
				Resource[] resources = ((ResourcePatternResolver) resourceLoader).getResources(location);
				// TODO NamespaceHandlerResolver 源头之一
			...
			// TODO NamespaceHandlerResolver 源头之一，reader是XmlBeanDefinitionReader
