package me.silentdoer.springbootrereadableinputstream.support;
...
public class ReuseInputStreamServletRequest extends HttpServletRequestWrapper {
    ...
    /**
     * 客户端是一定会对QueryString进行URLEncode的，因此如果要主动使用它应该进行URLDecode
     */
    ...
    public ReuseInputStreamServletRequest(HttpServletRequest request, Charset urlDecodeCharset) throws IOException {
        ...
        // 获得的是客户端发过来的原始数据已编码（经过Socket测试过，客户端会主动进行URLEncode不是tomcat去编码的）
    ...
    public String getParameter(@NonNull String name) {
        ...
        if(HttpMethod.POST.name().equalsIgnoreCase(this.methodType) && MediaType.APPLICATION_FORM_URLENCODED_VALUE.equalsIgnoreCase(this.contentType)){
    ...
    private void init() throws UnsupportedEncodingException {
        ...
        this.urlDecodedQueryString = URLDecoder.decode(this.queryString == null ? "" : this.queryString, this.urlDecodeCharset.name());
    }
